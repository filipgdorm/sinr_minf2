...
def calibrate_predictions(raw_preds):
    ...
    for col_idx in range(raw_preds.shape[1]):
        ...
        if os.path.exists(regressor_path):
            ...
            col_preds = regressor.predict_proba(col_value.reshape(-1, 1))[:, 1]  # Assuming binary classification
        else:
            # Handle case where no regressor file is found for this column value
            col_preds = torch.sigmoid(col_value).cpu().numpy()
