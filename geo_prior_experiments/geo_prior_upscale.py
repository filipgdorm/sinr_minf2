...
parser.add_argument("--gamma", type=float, default=0.1, help="gamma parameter norcal")

args = parser.parse_args()

MODEL_PATH = '../pretrained_models/' + args.model_path
RESULT_DIR = './upscale_results/'

if not os.path.exists(RESULT_DIR+args.exp_name+"/"+str(args.gamma)):
        os.mkdir(RESULT_DIR+args.exp_name+"/"+str(args.gamma))

# Set up logging to file
log_file_path = RESULT_DIR+args.exp_name+"/"+str(args.gamma)+"/log.out"
...
def run_evaluation(model, enc):
        ...
        for bb_id, bb in tqdm(enumerate(range(len(batch_start)-1)), total=len(batch_start)-1):
            ...
            geo_pred = torch.sigmoid(raw_preds) * args.gamma
