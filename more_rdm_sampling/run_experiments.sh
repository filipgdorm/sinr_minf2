            ...
            if [ -d "$SUBDIR" ]; then  # Check if it is a directory
                ...
                for MODEL_PATH in "$SUBDIR"/*; do
                    if [ -f "$MODEL_PATH" ]; then  # Check if it is a file
                        ...
                        python rdm_sampling.py --size_factor "${size_factor}" --model_path "${MODEL_PATH}" --result_dir "${o_result_dir}" --counter "${counter}"           
                        python evaluation.py --size_factor "${size_factor}" --model_path "${MODEL_PATH}" --result_dir "${o_result_dir}" --counter "${counter}"
