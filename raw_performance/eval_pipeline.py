...
def main(args):
    ...
    if args.method == "tgt_sampling":
         presence_absence, loc_feat_thresh, train_df_h3 = load_tgt_background_data(enc)
         with torch.no_grad():
            loc_emb_thresh = model(loc_feat_thresh, return_feats = True)
    ...
    for i, class_id in tqdm(enumerate(classes_of_interest), total=len(classes_of_interest)):
        ...
        elif args.method == "tgt_sampling": thres, fscore = target_sampling_threshold(loc_emb_thresh, wt_1, preds, species_locs, presence_absence, train_df_h3, class_id)
