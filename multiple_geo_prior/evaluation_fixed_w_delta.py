...
parser.add_argument("--counter", type=int, default=0, help="Experiment name")
parser.add_argument("--fixed_thres", type=float, default=0.1, help="Experiment name")
...
log_file_path = args.result_dir + f"/results/fixed_delta_log_{args.counter}.out"
...
def run_evaluation(model, enc):
    ...
    for bb_id, bb in tqdm(enumerate(range(len(batch_start)-1)), total=len(batch_start)-1):
        ...
        geo_pred = np.where(preds.numpy() > args.fixed_thres, 1.0, 0.01)
...
def report(results):
        ...
        results_file = args.result_dir + '/fixed_delta_mean_f1_scores.csv'
