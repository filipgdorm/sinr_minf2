...
log_file_path = args.result_dir + f"/results/continuous_delta_log_{args.counter}.out"
...
def run_evaluation(model, enc):
    ...
    for bb_id, bb in tqdm(enumerate(range(len(batch_start)-1)), total=len(batch_start)-1):
        ...
        geo_pred = np.clip(preds + 0.01, None, 1)
...
def report(results):
        ...
        results_file = args.result_dir + '/continuous_delta_mean_f1_scores.csv'
