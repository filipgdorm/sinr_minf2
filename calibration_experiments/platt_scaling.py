...
RESULT_DIR = './platt_scaling_results/'
...
for class_index, class_id in tqdm(enumerate(classes_of_interest), total=len(classes_of_interest)):
    ...
    calibrated_probs = logistic_regression.predict_proba(raw_preds.reshape(-1, 1))[:, 1]
    ...
    ##optimal scores
    optimal_fscore, optimal_thres = fscore_and_thres(y_test, calibrated_probs)

    #BINARISE RANGE MAP TGT BACKGROUND WITH CALIBRATED_PROBS
    gdfk["pred"] = calibrated_probs
    gdfk["raw_preds"] = raw_preds
    ...
    tgt_fscore = f1_at_thresh(y_test, calibrated_probs, tgt_thres, type='binary')
    ...
    row = {
        ...
        "optimal_fscore": optimal_fscore,
    }
    row_dict = dict(row)
    output.append(row_dict)
    #save calibrator
    model_filename = RESULT_DIR+args.exp_name+f'/calibrators/{taxa}_calibrator.pkl'
...
logging.info(f"Mean optimal_thres: {output_pd.optimal_thres.mean()}")
logging.info(f"Mean optimal_fscore: {output_pd.optimal_fscore.mean()}")