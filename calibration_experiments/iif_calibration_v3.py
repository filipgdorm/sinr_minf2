###scale sigmoid outside avtivation
import pandas as pd
import numpy as np
import json
import os
...
RESULT_DIR = f'./iif_calibration_results/v3/'
...
def iif_cal_activation(preds,iif):
    return (np.exp(preds)*iif) / ((np.exp(preds) *iif) + 1)
...
for class_index, class_id in tqdm(enumerate(classes_of_interest), total=len(classes_of_interest)):
    ...
    IF = IF_vals[class_id.item()]
    IIF = np.log(K/IF)

    preds = iif_cal_activation(raw_preds.numpy(), IIF)
